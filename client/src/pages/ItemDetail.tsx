  ...
  const handleExpressInterest = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error?.message || 'Failed to express interest');
      }
  ...
  if (error || !item) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Link to="/items" className="mt-4 inline-block text-primary-600 hover:text-primary-700">
          ← Back to items
        </Link>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="card">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <span className={`px-3 py-1 text-sm font-medium rounded-full ${getConditionBadgeColor(item.condition)}`}>
                    {item.condition}
                  </span>
                  <span className={`px-3 py-1 text-sm font-medium rounded-full ${getStatusBadgeColor(item.status)}`}>
                    {item.status}
                  </span>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-gray-900">{item.dimensions.weight} kg</span>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="flex items-center">
                      ...
                      {owner.isVerified && (
                        <svg className="w-5 h-5 text-primary-600 ml-1" fill="currentColor" viewBox="0 0 20 20">
            ...
            {isOwner && (
              <div className="flex space-x-3">
                <Link to={`/items/${id}/edit`} className="flex-1 btn-secondary text-center">
                  Edit Item
                </Link>
      ...
      {showContactModal && owner && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              To protect user privacy, please use the "Express Interest" feature to initiate contact.
              The owner will be notified and can respond through the platform.
            </p>
            <button
